...
def main():
    ...
    # 2) 학습된 ResNet18 모델 로드
    model = ResNet18(pretrained=False).to(device)
    state_dict = torch.load("resnet18_cifar10.pth", map_location=device)
    model.load_state_dict(state_dict)
    model.eval()


    # 3) Grad-CAM 대상 레이어 지정
    # ResNet18 클래스 안에 self.model이 torchvision resnet18 객체이므로:
    # 마지막 conv block = model.model.layer4[-1]
    ...
    plt.tight_layout()
    plt.savefig("plots/gradcam_resnet18_examples.png")
    print("Saved Grad-CAM examples to plots/gradcam_resnet18_examples.png")
